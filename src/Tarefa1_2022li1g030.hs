...
A função mapaValido pode ser definida em função de outras funções auxiliares: a função obstaculoTerrenoProprio, a função riosDirecaoOposta, a função compMaxObstaculos, 
a função umNenhumNoMinimo, a função larguraCompObstaculos e a função maxTerrenoContiguo, funções que recebem um mapa e retornam um bool e que verificam as restrições anteriormente 
enunciadas. 
Se uma destas funções auxiliares retornar False, ou seja, se uma das restrições não for respeitada, então automaticamente o mapa será inválido, ou seja, a função mapaValido 
irá retornar False. Assim, para o mapa ser válido, ou seja, para a função mapaValido retornar True, então todas as restrições terão de ser respeitadas, 
logo todas as funções auxiliares terão de retornar True.

Assim, a função mapaValido pode ser definida da seguinte forma:

@
mapaValido :: Mapa -> Bool
mapaValido (Mapa n l) = obstaculoTerrenoProprio (Mapa n l) && riosDirecaoOposta (Mapa n l) && compMaxObstaculos (duplicarListaObstaculos (Mapa n l)) && umNenhumNoMinimo (Mapa n l) && larguraCompObstaculos (Mapa n l) && maxTerrenoContiguo ((Mapa n l)
@

A função mapaValido contém ainda outra função auxiliare, a função duplicarListaObstaculos que por sua vez
-}

mapaValido :: Mapa -> Bool
mapaValido (Mapa n l) = obstaculoTerrenoProprio (Mapa n l) && riosDirecaoOposta (Mapa n l) && compMaxObstaculos (duplicarListaObstaculos (Mapa n l)) && umNenhumNoMinimo (Mapa n l) && larguraCompObstaculos (Mapa n l) && maxTerrenoContiguo (Mapa n l)

{- |A função obstaculoTerrenoProprio, que recebe um mapa e retorna um bool, verifica se num dado mapa não existem obstáculos em terrenos impróprios, ou seja,
verifica que não existem Troncos em Estradas ou Relvas, que não existem Carros em Rios ou Relvas e que não existem Arvores em Rios ou Estradas.
Esta função percorre toda a lista de obstáculos á procura de um obstáculo impróprio para o terreno dado. Se encontrar um obstáculo impŕoprio para o terreno, a função retornara False.
Se não encontrar, então a função irá chamar a ela mesma (função recursiva) para procurar na próxima lista de obstáculos um obstáculo impróprio para o terreno dado.
Quando a função tiver percorrido todas as listas de obstáculos do Mapa sem encontrar nenhum obstáculo impróprio para o terreno dado e quando apenas sobrar a lista vazia,
a função irá retornar True.
...
Depois usei a função mapaListaObstaculosDuplicados, que recebe um mapa e retorna um mapa, e que obtém o mapa com as listas de obstáculos duplicados.
...
se não há 6 troncos seguidos ou 4 carros seguidos na próxima lista de obstáculos. Quando a função percorrer todo o mapa e não encontrar 6 troncos seguidos ou 4 carros seguidos em
nenhuma das listas de obstáculos e quando apenas sobrar a lista vazia, a função irá retornar True.
...
{- |A função larguraCompObstaculos, que recebe um mapa e retorna um bool, verifica se num dado mapa a largura do mapa é igual ao comprimento de todas as listas de obstáculos.
Esta função calcula o comprimeto da lista de obstáculos através da função length pré-definida no Haskel e verifica se esse valor é igual á largura do mapa. Se não for igual, 
a função retornara False. Se for igual, então a função irá chamar a ela mesma (função recursiva) para calcular o comprimento da próxima lista de obstáculos  e comparar esse valor com
a largura do mapa. Quando a função tiver percorrido todas as listas de obstáculos do Mapa sem encontrar nenhuma lista de comprimento diferente à largura e quando apenas 
sobrar a lista vazia, a função irá retornar True.
...
a função irá chamar a ela mesma (função recursiva) para verificar que os próximos 5 terrenos não são todos Rio ou que os próximos 6 terrenos não são todos Estrada ou Relva. Quando a
função percorrer toda a lista do mapa sendo que não existem contíguamente mais do que 4 rios, nem 5 estradas ou relvas, e quando apenas sobrar a lista vazia, a função irá retornar True.
